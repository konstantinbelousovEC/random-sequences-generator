    ...
    class Generator final {
     ...
     public:
        ...
        Container generate(size_type<Container> size,
            ...
            for (size_type<Container> i = 0; i < size; ++i) {
                if constexpr (sfinae::has_push_back_method<Container>()) {
                    container.push_back(val_gen(gen_));
                } else if constexpr (sfinae::has_push_front_method<Container>()) {
        ...
        template<typename Container,
                 typename KeyGenerator = ArithmeticValueGenerator<key_type<Container>>,
                 typename ValueGenerator = ArithmeticValueGenerator<mapped_type<Container>>,
